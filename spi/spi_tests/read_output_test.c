...
//  read_from_spi(DEVICE_1, GPIOB, res);
  write2spi(DEVICE_0, GPIOA, 0xcf);
 // printf("res = 0x%x, 0x%x, 0x%x\n", res[0], res[1], res[2]);
 // write2spi(DEVICE_0, GPIOA, 0xff);
  read_from_spi(DEVICE_0, GPIOA, res);
...
void write2spi(unsigned char device, unsigned char regaddr, unsigned char tx_data)
{
  ...
  buf[1] = regaddr;
  buf[2] = tx_data;
  printf("[BEFORE WRITE] device:0x%x, regaddr:0x%x, data:0x%x\n", buf[0], buf[1], buf[2]);
  digitalWrite(SS_PORT, 0);
  wiringPiSPIDataRW(SPI_CHANNEL, buf, sizeof(buf));
  digitalWrite(SS_PORT, 1);

  printf("[AFTER WRITE] device:0x%x, regaddr:0x%x, data:0x%x\n", buf[0], buf[1], buf[2]);
}
...
void read_from_spi(unsigned char device, unsigned char regaddr, unsigned char* res)
{
  ...
  printf("[BEFORE READ] device:0x%x, regaddr:0x%x\n", res[0], res[1]);
  digitalWrite(SS_PORT, 0);
  wiringPiSPIDataRW(SPI_CHANNEL, res, sizeof(res));
  digitalWrite(SS_PORT, 1);

  printf("[AFTER READ] buf[0]:0x%x, buf[1]:0x%x, buf[2]:0x%x\n", res[0], res[1], res[2]);
}
